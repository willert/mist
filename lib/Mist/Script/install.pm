...
if ( $parent ) {
  ...
  # perllocal.pod is appended to in place; left hard-linked, an install into the
  # branch would write through the shared inode and mutate the parent too. Break
  # the link so the branch gets its own file - it is an install log, not
  # load-bearing, so letting it regenerate fresh is harmless.
  require File::Find;
  File::Find::find( sub { unlink $_ if $_ eq 'perllocal.pod' }, $LOCAL_LIB_DIR );
}
...
# Installing a *different* perl than the active one builds into its own dir,
# body and rc and never touches the selector until the final atomic flip, so it
# is atomic for free. Re-installing the active perl mutates its one lib dir
# live, so during that build the selector is stubbed to fail loudly.
my $active_body   = readlink $mist_run_fn;
my $is_cross      = defined( $active_body ) && $active_body ne $body_name;
my $stub_selector = $activate && !$is_cross;
...
if ( $stub_selector ) {
  unlink $mist_run_fn;   # may be a symlink to an old body; never write through it
  open my $stub, '>', $mist_run_fn
    or die "Creating $mist_run_fn failed: $!";
  print $stub <<'BASH';
#!/bin/bash
...
# point the branch-less generic lib dir at this branch. The target is the branch
# dir's bare basename (both are siblings under perl5/), so it resolves natively -
# for the wrapper and local::lib, not only for mist's own readlink.
if ( $branch ) {
  if ( -l $generic_libdir or not -e $generic_libdir ) {
    my ( $branch_dir_name ) = ( File::Spec->splitdir( $LOCAL_LIB_DIR ) )[-1];
    print "Re-linking $generic_libdir to current branch\n";
    unlink $generic_libdir;
    symlink $branch_dir_name, $generic_libdir
